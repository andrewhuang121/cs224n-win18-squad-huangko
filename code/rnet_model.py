...
class RNet(object):
    ...
    def build_graph(self):
        ...
        with tf.variable_scope('encoding') as scope:
            ...
            c_output, c_fwd_output, c_back_output = tf.contrib.rnn.static_bidirectional_rnn(emb_fwd_cell, emb_back_cell, 
                    unstacked_context, tf.reduce_sum(self.context_mask, reduction_indices=1), dtype='float32')

            tf.get_variable_scope().reuse_variables()

            qn_output, q_fwd_output, q_back_output = tf.contrib.rnn.static_bidirectional_rnn(emb_fwd_cell, emb_back_cell, 
                    unstacked_context, tf.reduce_sum(self.qn_mask, reduction_indices=1), dtype='float32')
            ...
            u_Q = tf.nn.dropout(u_Q, self.FLAGS.keep_prob)
            u_P = tf.nn.dropout(u_P, self.FLAGS.keep_prob)
        ...
        with tf.variable_scope('Attention_gated') as scope:
            for t in range(0, self.FLAGS.context_len): 
                ...
                QP_cell = tf.contrib.rnn.DropoutWrapper(tf.contrib.rnn.GRUCell(self.FLAGS.encode_size), self.FLAGS.keep_prob)
                ...
                a_t = masked_softmax(s_t, self.qn_mask, 1) # [batch, q_len]
            ...
            v_P = tf.nn.dropout(v_P, self.FLAGS.keep_prob)
        ...
        with tf.variable_scope("self_matching_attn") as scope:
            ...
            for t in range(0, self.FLAGS.context_len):
                ...
                #a_t = tf.nn.softmax(s_t, 1)
                a_t = masked_softmax(s_t, self.context_mask, 1)
            ...
            SM_input=  tf.stack(SM_input, 1) # [batch, context_len, 2 * encode_size]
            SM_fwd_cell = tf.contrib.rnn.DropoutWrapper(tf.contrib.rnn.GRUCell(self.FLAGS.encode_size), self.FLAGS.keep_prob)
            SM_back_cel = tf.contrib.rnn.DropoutWrapper(tf.contrib.rnn.GRUCell(self.FLAGS.encode_size), self.FLAGS.keep_prob)
            h_P, SM_final = tf.nn.bidirectional_dynamic_rnn(SM_fwd_cell, SM_back_cell, SM_input, tf.reduce_sum(self.context_mask, reduction_indices=1), dtype=tf.float32)

            h_P = tf.nn.dropout(h_P, self.FLAGS.keep_prob) #[batch, context_len, 2*encode_size]

        # OUTPUT
        with tf.variable_scope("Output") as scope:
            W_ruQ = tf.get_variable('W_ruQ', shape=(2*self.FLAGS.encode_size, 2*self.FLAGS.encode_size))   
            V_rQ = tf.get_variabel('V_rQ', shape=(self.FLAGS.question_len, 2*self.FLAGS.encode_size))
            W_vQ = tf.get_variable('W_vQ', shape=(2*self.FLAGS.encode_size, 2*self.FLAGS.encode_size))

            v_rQ = tf.get_variable('v_rQ', shape=(2 * self.FLAGS.encode_size))

            WuQ_ujQ = tf.matmul(u_Q, W_ruQ) #[batch, q_len, 2 * encode_size]
            WvQ_VrQ = tf.matmul(V_rQ, W_vQ) #[q_len, 2*encode_size]

            s_t = tf.tensordot(tf.tanh(WuQ_ujQ + WvQ_VrQ), v_rQ, axes=[[2],[0]]) # The addition will broadcast # final shape: [batch, q_len]
            a_t = masked_softmax(s_t, self.qn_mask, 1)
            rQ = tf.einsum('ij,ijk->ik', a_t, u_Q)
            rQ = tf.nn.dropout(rQ, self.FLAGS.keep_prob) #[batch, 2*encode_size]
            ...
            for t in range(0,2): 
                ...
                W_hP = tf.get_variable('W_hP', shape=(2*self.FLAGS.encode_size, self.FLAGS.encode_size))
                W_ha = tf.get_variable('W_ha', shape=(2*self.FLAGS.encode_size, self.FLAGS.encode_size))
                v_ap = tf.get_variable('v_ap', shape=(self.FLAGS.encode_size)) # answer pointer bias

                WhP_hP = tf.matmul(h_P, W_hP) #[batch, context_len, encode_size]
                Wha_ha = tf.reshape(tf.matmul(h_a, W_ha), (self.FLAGS.batch_size, 1, -1)) #[batch, 1, encode]

                ans_cell = tf.contrib.rnn.DropoutWrapper(tf.contrib.rnn.GRUCell(2 * self.FLAGS.encode_size), self.FLAGS.keep_prob)

                s_t = tf.tensordot(tf.tanh(WhP_hP + Wha_ha), v_rQ, axes=[[2], [0]]) # [batch, context_len]
                #a_t = tf.nn.softmax(s_t, 1)
                a_t = masked_softmax(s_t, self.context_mask, 1)
